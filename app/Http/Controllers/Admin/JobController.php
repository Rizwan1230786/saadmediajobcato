...
use App\Traits\CustomJobsTrait;
use App\Traits\OtherJobsTrait;
...
class JobController extends Controller
{
    ...
    public function indexAdminJobs()
    {

        $companies = Company::where('is_admin', 1)->with(['jobs'])->get()->pluck('id');
        $jobs = Job::whereIn('company_id', $companies)->get();

        return view('admin.job.index-2')
                            ->with('jobs', $jobs);
        ...
        return Datatables::of($jobs)
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						...
						<li>
							<a href="' . route('delete.job', ['id' => $jobs->id]) . '" class=""><i class="fa fa-trash-o" aria-hidden="true"></i>Delete</a>
						</li>
						<li>
    ...
    private function deleteOlderCustomJobAttachment($id)
    {
        try {
            $custom_job = OtherWebsiteJob::findOrFail($id);
            $image = $custom_job->logo;
            if (!empty($image)) {
                File::delete(ImgUploader::real_public_path() . 'other_jobs/' . $image);
            }
            return 'ok';
        } catch (ModelNotFoundException $e) {
            return 'notok';
    ...
    public function deleteJob($id){
        ...
        $jobscount=Job::where('country_id',$job->country_id)->count();
        if(isset($jobscount) && $jobscount <=1){
            Country::where('id',$job->country_id)->update(['status'=>0]);
        }
        $job->delete();
