...
class CompanyController extends Controller
{
    ...
    public function companyProfile()
    {
        ...
        if( !empty($company->id) ){
            $verification_files = VerificationFile::where('company_id', $company->id)->get();
            foreach($verification_files as $index => $file){
                $verification_files[$index]->download = route('fileDownload')."?path={$file->full_path}";
            }
        ...
        return view('company.edit_profile')
                        ...
                        ->with('verifications', $verification_files);
    }
    ...
    public function updateCompanyProfile(CompanyFrontFormRequest $request)
    {
        ...
        if( !$company->is_verification_cleared ){
            $verification_files = [];
            $verification_names = [];
            if( $request->hasFile('verification_files') ){
                $verification_files = $request->file('verification_files');
                $verification_names = $request->get('verification_names');
                foreach($verification_files as $index => $file){
                    // $file = $request->video_attachment;
                    $fileName = time().'.'.$file->getClientOriginalExtension();
                    if( !empty($verification_names[$index]) ){
                        $fileName = $verification_names[$index].'_'.$company->id.'.'.$file->getClientOriginalExtension();
                    }
                    $full_path_name = 'company_verification/'.$fileName;
                    $temp_path = $file->storeAs(
                        'company_verification', $fileName
                    );
                    // $temp_path = $file->store($fileName, 'public');
                    //$destinationPath = public_path('/company_verification');
                    // $file->move($destinationPath,$fileName);
                    $verification_files[$index] = ['full_path' => $temp_path, 'file_name' => $fileName];
                }
            }
            if( !empty($verification_files)){
                $previous_files = VerificationFile::where('company_id', $company->id)->get();
                foreach($previous_files as $file){
                    Storage::disk('public')->delete($file->full_path);
                    Storage::disk('public')->deleteDirectory($file->file_name);
                    $file->delete();
                }
                foreach($verification_files as $file){
                    $file_record = new VerificationFile;
                    $file_record->full_path = $file['full_path'];
                    $file_record->file_name = $file['file_name'];
                    $file_record->company_id = $company->id;
                    $file_record->status = false;
                    $file_record->save();
                }            
            }
        }
    ...
    public function postedJobs(Request $request)
    {
        ...
        foreach($jobs as $index => $job){
            $job->applied = JobApply::where('job_id', $job->id)->count();
        }
