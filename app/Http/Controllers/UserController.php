...
class UserController extends Controller
{
    ...
    public function getUserData($user_id, Request $request)
    {
        $user = User::findOrFail(Auth::user()->id);
        return response()->json(array('success' => true, 'status' => 200, 'data' => $user), 200);

    }

    public function updateProfileVideo($user_id, Request $request)
    {
        
        $user = User::findOrFail(Auth::user()->id);
        /*         * ************************************ */
        if ($request->get('video_link')) {
            $user->video_link = $request->get('video_link');
            if (!$request->hasFile('video_attachment')) {
            $user->video_attachment = null;
            }
        }
        /*         * **************************************** */
        // if ($request->hasFile('video_attachment')) {
        ...
        //     $fileName = ImgUploader::UploadImage('user_video', $image, $request->input('name'), 300, 300, false);
        //     $user->video_attachment = $fileName;
        // }
       // return $request->video_attachment;
        if ($request->hasFile('video_attachment')) {
         $file = $request->video_attachment;
            $fileName = time().'.'.$file->getClientOriginalExtension();
            $destinationPath = public_path('/user_video');
        ...
        $user->update();
        flash(__('Intro video updated successfully'))->success();
