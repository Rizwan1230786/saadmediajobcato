...
Route::get('/getip', function () {
    ...
    $info = [];
    if (!empty($ip_address)) {
        $info = file_get_contents("http://api.ipstack.com/" . $ip_address . "?access_key=12858971147fa3a6425ad395fd229392");
    }
    echo json_encode(['IP' => $ip_address, 'info' => $info]);
})->name('clientIP');
...
Route::get('/getFile', function () {
    ...
    if ($request['path']) {
        try {
            return \Storage::download("app/{$request['path']}");
...
Route::get('PerformMigration', function () {
    try {
        \Artisan::call('migrate', ['--path' => 'database/migrations', '--database' => 'mysql', '--force' => true]);
