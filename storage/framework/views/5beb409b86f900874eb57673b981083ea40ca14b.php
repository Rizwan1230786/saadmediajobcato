            ...
            <div class="col-md-12"> 
                ...
                <div class="portlet light portlet-fit portlet-datatable bordered">
                    <div class="portlet-title">
                        ...
                        <div class="actions"> <a href="<?php echo e(route('create.state')); ?>" class="btn btn-xs btn-success"><i class="glyphicon glyphicon-plus"></i> Add New State</a> </div>
                    </div>
                                    ...
                                    <thead>
                                        <tr role="row" class="filter">
                                            ...
                                            <td>
                                                ...
                                                <?php echo Form::select('country_id', ['' => 'Select Country']+$countries, $default_country_id, array('id'=>'country_id', 'class'=>'form-control')); ?></td>
...
<script>
    $(function () {
        var oTable = $('#stateDatatableAjax').DataTable({
             ...
             */
            ajax: {
                url: '<?php echo route('fetch.data.states'); ?>',
                data: function (d) {
                    d.lang = $('#lang').val();
    ...
    function deleteState(id, is_default) {
        ...
        if (confirm(msg)) {
            $.post("<?php echo e(route('delete.state')); ?>", {id: id, _method: 'DELETE', _token: '<?php echo e(csrf_token()); ?>'})
    ...
    function makeActive(id) {
        $.post("<?php echo e(route('make.active.state')); ?>", {id: id, _method: 'PUT', _token: '<?php echo e(csrf_token()); ?>'})
                .done(function (response) {
                    if (response == 'ok')
                    {
                        var table = $('#stateDatatableAjax').DataTable();
    ...
    function makeNotActive(id) {
        $.post("<?php echo e(route('make.not.active.state')); ?>", {id: id, _method: 'PUT', _token: '<?php echo e(csrf_token()); ?>'})
                .done(function (response) {
                    if (response == 'ok')
                    {
                        var table = $('#stateDatatableAjax').DataTable();
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('admin.layouts.admin_layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\JobCato\resources\views/admin/state/index.blade.php ENDPATH**/ ?>