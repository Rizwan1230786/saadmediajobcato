            ...
            <div class="col-md-12"> 
                ...
                <div class="portlet light portlet-fit portlet-datatable bordered">
                    <div class="portlet-title">
                        ...
                        <div class="actions"> <a href="<?php echo e(route('create.job.experience')); ?>" class="btn btn-xs btn-success"><i class="glyphicon glyphicon-plus"></i> Add New Job Experience</a> </div>
                    </div>
...
<script>
    $(function () {
        var oTable = $('#jobExperienceDatatableAjax').DataTable({
             ...
             */
            ajax: {
                url: '<?php echo route('fetch.data.job.experiences'); ?>',
                data: function (d) {
                    d.lang = $('#lang').val();
    ...
    function deleteJobExperience(id, is_default) {
        ...
        if (confirm(msg)) {
            $.post("<?php echo e(route('delete.job.experience')); ?>", {id: id, _method: 'DELETE', _token: '<?php echo e(csrf_token()); ?>'})
    ...
    function makeActive(id) {
        $.post("<?php echo e(route('make.active.job.experience')); ?>", {id: id, _method: 'PUT', _token: '<?php echo e(csrf_token()); ?>'})
                .done(function (response) {
                    if (response == 'ok')
                    {
                        var table = $('#jobExperienceDatatableAjax').DataTable();
    ...
    function makeNotActive(id) {
        $.post("<?php echo e(route('make.not.active.job.experience')); ?>", {id: id, _method: 'PUT', _token: '<?php echo e(csrf_token()); ?>'})
                .done(function (response) {
                    if (response == 'ok')
                    {
                        var table = $('#jobExperienceDatatableAjax').DataTable();
